...
- Default `site` data lives in `src/_data/site.ts` (full set: `url`, `base_path`, `title`, `repo_url`, `houseTheme`). The function receives `configData.site` and merges any `addGlobalData("site", ...)` overrides over the defaults — so the publish config only needs to provide the keys that differ. **Keep `_data/site.ts` authoritative**: when splitting config into default + override layers, the default side must hold the complete current value set; the override side specifies only the keys that actually differ.
  - `site.houseTheme` names the theme slug (a `packages/themes/src/<slug>/` directory) used as the website's own chrome/baseline CSS — the single source of truth referenced by `nav.html`, `index.html`, `docs.html`, `docs-index.html`, and the homepage Quickstart, so renaming or swapping the house theme only requires updating this one value.
...
- **Layout via directory data.** `src/docs/docs.json` (`{ "layout": "docs.html" }`) applies `_layouts/docs.html` to every docs page, so a page needs only `title` / `description` frontmatter. `docs.html` is `index.html`'s chrome minus the theme gallery and loads the site's own house theme's `theme.min.css` (the theme slug named by `site.houseTheme` in `src/_data/site.ts`).
