...
`build.ts` walks `src/` directories and emits a single artifact per theme by compiling the shared entry file `packages/themes/entry.scss` with `sass.compile` once per theme (`loadPaths: [src/<theme>]`, so `@use "_variables"` resolves to that theme):

| Output (release / debug) | Entry (`entry.scss`) | Use case |
| --- | --- | --- |
| `theme.min.css` / `theme.css` | `@use "pkg:bulma"; @use "_variables" as v; :root { @include v.variables; }` | Bundle Bulma + theme; a drop-in replacement for vanilla Bulma. |
